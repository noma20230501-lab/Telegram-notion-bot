    ...
    def parse_property_info(
        ...
        for line in lines[start_idx:]:
            ...
            # 4. 건축물용도 / 면적 (복층/통상가 지원)
                ...
                # 2단계: 상세 패턴 파싱 (예: "1층 계약48.43㎡ 전용48.43㎡ 14평")
                상세_패턴 = re.findall(
                    r'(\d+)층[^/]*?'
                    r'계(?:약)?(?:면적)?\s*(\d+\.?\d*)\s*(?:m2|㎡)?[^/]*?'
                    r'전(?:용)?(?:면적)?\s*(\d+\.?\d*)\s*(?:m2|㎡)?[^/]*?'
                ...
                if 층별_정보:
                    ...
                    층별_평수 = [f"{층}층 {층별_정보[층]['평']}평" for 층 in sorted_floors]
...
class NotionUploader:
    ...
    def _build_notion_properties(
        ...
        if 범위_match:
            ...
            # 콤마 구분 형식: "1,2,3층" 또는 개별: "1층"
            층_matches = re.findall(r'(\d+)층', 주소)
            if 층_matches:
                층_list = [f"{층}층" for 층 in 층_matches]
